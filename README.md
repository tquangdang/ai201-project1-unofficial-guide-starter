...
**This failure is phrasing-sensitive, which sharpens the diagnosis.** The same question without the course code - "How many exams does Huang's Analysis of Algorithms course have?" - retrieves the right chunk and answers correctly ("two: a midterm and a final"). Adding "(CS4713)" is what breaks it: the course code resembles the many CS#### codes scattered across every file, so it pulls retrieval toward course-code-heavy chunks, while the plain phrase "Analysis of Algorithms" matches the exact wording in Huang's review. So the deeper root cause is that retrieval is sensitive to query phrasing - not that the answer is missing.
